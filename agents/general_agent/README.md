...
### Issue: "input_tokens=0, output_tokens=0 — No actual LLM output"

**Symptoms**: All subprocesses complete in 2-3 seconds, logs show `[Claude Usage]: input_tokens=0, output_tokens=0`, every iteration gets `score=0.0000`, and `executor/work_dir/` is always empty.

**Root cause**: The model name in `task_config.yaml` doesn't exist at your API endpoint. The bundled Claude Code CLI connects but the model returns nothing.

**Fix**:
1. Verify the model name is correct for your endpoint:
   ```yaml
   # task_config.yaml
   llm_config:
     model: "anthropic/deepseek-v4-pro"  # Use a model that actually exists!
   ```
2. Confirm `ANTHROPIC_BASE_URL` points to the right endpoint:
   ```bash
   echo $ANTHROPIC_BASE_URL
   # e.g., https://api.deepseek.com/v1 or https://api.anthropic.com
   ```
3. Check logs: `output-<task>/logs/evolux.log`

### Issue: "WSL — no output or process hangs"

1. Create `.venv` inside WSL filesystem (not `/mnt/d/...`):
   ```bash
   uv venv .venv --python 3.12
   source .venv/bin/activate
   uv pip install -e .
   ```
2. Export environment variables in WSL shell:
   ```bash
   export ANTHROPIC_API_KEY="your-key"
   export ANTHROPIC_BASE_URL="your-endpoint"
   ```
3. The bundled Claude Code CLI is Linux-compatible — it works in WSL if installed correctly.

### Issue: "Plan file not found / Summary file not found"

This is typically caused by the model returning no content (see token=0 issue above). If tokens ARE non-zero, check that `max_turns` is high enough for the agent to complete its work.

### Issue: "Output directory structure doesn't match docs"

The actual structure uses **UUID-based task directories** and **numeric iteration directories**:
```
output-todo-list/
├── logs/                          # Log files
├── database/                      # Evolution database
└── <uuid>/                        # e.g., 6bb70103-27aa-49fd-862b-26a329500f15
    ├── 1/                         # Iteration 1
    │   ├── planner/
    │   ├── executor/
    │   │   ├── work_dir/          # Generated code
    │   │   └── evaluator_dir/     # Evaluation results
    │   └── summarizer/
    └── 2/                         # Iteration 2
```
**Use the [visualizer](#-visualization)** to browse results — it handles the structure automatically.
